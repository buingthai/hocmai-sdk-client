 ...
 *
 */
namespace Hocmai\Authentication;
use Hocmai\Exceptions\HocmaiResponseException;
use Hocmai\Exceptions\HocmaiSDKException;
use Hocmai\Hocmai;
use Hocmai\HocmaiApp;
use Hocmai\HocmaiClient;
use Hocmai\HocmaiRequest;
...
class OAuth2Client
{
     ...
     */
    const BASE_AUTHORIZATION_URL = 'http://oauth2.local:8000';
     ...
     *
     * @var HocmaiRequest|null
     */
    protected $lastRequest;

    /**
     * @param HocmaiApp    $app
     * @param HocmaiClient $client
     * @param string|null    $graphVersion The version of the Graph API to use.
     */
    public function __construct(HocmaiApp $app, HocmaiClient $client, $graphVersion = null)
    {
        $this->app = $app;
        $this->client = $client;
//        $this->graphVersion = $graphVersion ?: Facebook::DEFAULT_GRAPH_VERSION;
    ...
    public function debugToken($accessToken)
    {
        ...
        $this->lastRequest = new HocmaiRequest(
            $this->app,
    ...
    public function getAuthorizationUrl($redirectUrl, $state, array $scope = [], array $params = [], $separator = '&')
    {
        $params += [
            ...
            'state' => $state,
            'response_type' => 'code',
            'sdk' => 'php-sdk-' . Hocmai::VERSION,
            'redirect_uri' => $redirectUrl,
            'scope' => implode(',', $scope)
        ];

        return static::BASE_AUTHORIZATION_URL . '/' .
        $this->graphVersion . '/dialog/oauth?' . http_build_query($params, null, $separator);
    }
    ...
    public function getCodeFromLongLivedAccessToken($accessToken, $redirectUri = '')
    {
        ...
        if (!isset($data['code'])) {
            throw new HocmaiSDKException('Code was not returned from Graph.', 401);
        }
    ...
    protected function requestAnAccessToken(array $params)
    {
        ...
        if (!isset($data['access_token'])) {
            throw new HocmaiSDKException('Access token was not returned from Graph.', 401);
        }
     ...
     *
     * @return HocmaiResponse
     *
     * @throws HocmaiResponseException
     */
